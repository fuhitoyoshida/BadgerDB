...
namespace badgerdb
{
	...
	class BTreeIndex {
		...
		private:
			...
			////////////////////////// OUR FIELD AND FUNCTION //////////////////////////////////////////////////////////////////
			
			// true if B+tree only has root 
			bool onlyRoot; 
			...
			* RP: RidKeyPair
			*/	
			template<class LT,class RP> void insertLeaf(LT* node, RP pair);
			...
			* PP: PageKeyPair
			*/
			template<class NT,class PP> void insertNonLeaf(NT* node, PP pair);
			
			/*
			* split a leaf node
			* newPair is the splitted node
			* 
			* LT: leafNode
			* PP: PageKeyPair
			* RP: RidKeyPair
			*/
			template<class LT,class PP,class RP> void splitLeaf(LT* leafNode, RP RIDPair, PP& newPair);

			/*
			* split a non leaf node
			* newPKPair is the splitted node
			*
			* NT: nonLeafNode
			* PP: PageKeyPair
			*
			*/
			template<class NT,class PP> void splitNonLeaf(NT* leafNode, PP returnP, PP& newPKPair);
			...
			*
			*/
			template<class LT,class NT,class PP,class RP> void createNewRoot(PageId oldNo, PP newPair, int level);
			

			/*
			* start the whole process to insert
			* 
			* T: attributeType
			* LT: leafNode
			* NT: nonLeafNode
			...
			*
			*/
			template<class T, class LT,class NT,class PP,class RP> void start(PageId currPageNo, PP& newPPair, RP newRPair);

			
			/*
			* assign the right value. In order to compile in template function, we have to split it into two functions.
			* one for int and double, and one for string.
			*
			*/
			template<class T> void insertS (T a, T b);
			void insertID (void* a, void* b);
			
			/*
			* Find the position according to page number.
			*
			*/
			template<class T, class LT> int leafPos(PageId currNo, T lowVal);
			
			/*
			* Find the position according to page number.
			*
			*/
			template<class T,class NT> int nonLeafPos(PageId currNo, T lowVal);
			 ...
			 * 
			 */
			template<class T, class LT,class NT,class PP,class RP> void search(T lowVal);


			/**
			* compare different type of key.
			*
			*/
			template<class T> int compare(T a, T b);


			///////////////////////////// END OF OUR FUNCTION AND FIELD ///////////////////////////////////////////////////
